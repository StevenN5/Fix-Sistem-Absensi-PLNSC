<nav class="bg-pln-green shadow-md sticky top-0 z-50 w-full">
    <div class="max-w-[1920px] mx-auto px-4 sm:px-6 md:px-12 py-3 md:py-4 flex justify-between items-center">
        <a href="{{ route('magang.landing') }}" class="hover:opacity-90 transition flex-shrink-0">
        ...
        <div class="hidden md:flex items-center gap-6">
            <a href="{{ route('magang.landing') }}" class="text-white font-medium hover:text-pln-yellow transition shadow-sm">
                Beranda
            </a>
            
            <a href="{{ route('magang.admin.login') }}" class="bg-white/10 backdrop-blur-sm border border-white/30 text-white px-5 py-2 rounded-full font-medium hover:bg-white hover:text-pln-green transition">
                Login 
            </a>
        </div>
    ...
    <div id="mobileNavMenu" class="hidden md:hidden border-t border-white/20 bg-pln-dark/95 backdrop-blur-sm">
        <div class="max-w-[1920px] mx-auto px-4 sm:px-6 py-4 space-y-3">
            <a href="{{ route('magang.landing') }}" class="block text-white font-medium py-2.5 px-4 rounded-lg hover:bg-white/10 transition">
                🏠 Beranda
            </a>
            <a href="{{ route('magang.admin.login') }}" class="block text-center bg-white/10 backdrop-blur-sm border border-white/30 text-white py-2.5 px-4 rounded-lg font-medium hover:bg-white hover:text-pln-green transition">
                🔐 Login Admin
            </a>
        </div>
    </div>
