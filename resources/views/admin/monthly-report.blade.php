...
@section('breadcrumb')
    <div class="col-sm-6">
        <h4 class="page-title text-left">{{ __('global.monthly_report') }}</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="javascript:void(0);">{{ __('global.home') }}</a></li>
            <li class="breadcrumb-item"><a href="javascript:void(0);">{{ __('global.monthly_report') }}</a></li>
        </ol>
    </div>
@endsection
            ...
            <div class="card admin-card">
                <div class="card-body">
                    <div class="export-box mb-3 d-flex align-items-center justify-content-between flex-wrap">
                        ...
                        <form id="monthly-report-export-form" class="form-inline">
                            ...
                            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap admin-table" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-priority="4">{{ __('global.monthly_report_document') }}</th>
                                        <th data-priority="5">{{ __('global.uploaded_by') }}</th>
                                        <th data-priority="6">{{ __('global.file_size') }}</th>
                                        <th data-priority="7">{{ __('global.month') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($reports as $report)
                                        <tr>
                                            ...
                                            <td>
                                                <a class="file-pill" target="_blank" rel="noopener" href="{{ route('monthly-report.view', $report->id) }}">
                                                    Lihat
                                                </a>
                                                <a class="file-pill" href="{{ route('monthly-report.download', $report->id) }}">
                                                    {{ $report->file_name }}
                                                </a>
                                            </td>
...
@section('script')
    <script>
        $(function() {
            $('#monthly-report-export-form').on('submit', function(e) {
                e.preventDefault();
                var month = $(this).find('[name="month"]').val();
                if (!month) {
                    return;
                }
                window.location.href = "{{ url('/monthly-report/export') }}/" + month;
            });
        });
    </script>
@endsection
