...
@endphp
    @include('layouts.user-navbar', ['active' => 'final'])
    <div class="attendance-page">
        <div class="attendance-shell">
            ...
            <div class="hero-card">
                <div>
                    <div class="hero-title">Laporan Akhir Magang</div>
                    <div class="hero-subtitle">Unggah dokumen laporan akhir dan cek riwayat unggahan.</div>
                    @if ($errors->any())
                ...
                <div>
                    @if ($lastFinalReport)
                        <div class="alert alert-info mb-2">
                            Dokumen terakhir diunggah: {{ optional($lastFinalReport->created_at)->format('d-m-Y H:i') }}
                        </div>
                        <a class="btn btn-outline-primary action-button mb-2" target="_blank" rel="noopener"
                            href="{{ route('final-report.view', $lastFinalReport->id) }}">
                            Lihat Laporan Akhir Terakhir
                        </a>
                        <a class="btn btn-outline-secondary action-button" href="{{ route('final-report.download', $lastFinalReport->id) }}">
                            Unduh Laporan Akhir Terakhir
                        </a>
                    @else
            ...
            <div class="action-card">
                <div class="action-header">
                    <div class="action-title">Unggah Laporan Akhir</div>
                    <span class="action-status {{ $lastFinalReport ? 'done' : 'pending' }}">
                        {{ $lastFinalReport ? 'Tercatat' : 'Belum' }}
                    </span>
                </div>
                <form method="POST" action="{{ route('final-report.store') }}" class="action-form" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="final_report">Unggah Laporan Akhir (PDF)</label>
                        <input type="file" class="form-control" id="final_report" name="final_report" accept=".pdf" required>
                    </div>
                    <div class="form-group">
                        <label for="what_learned">Apa yang Dipelajari</label>
                        <textarea class="form-control" id="what_learned" name="what_learned" rows="3"
                            placeholder="Tuliskan pembelajaran utama selama magang.">{{ old('what_learned') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="challenges">Kendala</label>
                        <textarea class="form-control" id="challenges" name="challenges" rows="3"
                            placeholder="Tuliskan kendala yang dihadapi.">{{ old('challenges') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="breakthrough_solution">Breakthrough / Solusi</label>
                        <textarea class="form-control" id="breakthrough_solution" name="breakthrough_solution" rows="3"
                            placeholder="Tuliskan solusi atau terobosan yang diterapkan.">{{ old('breakthrough_solution') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="suggestions_for_company">Masukan / Rekomendasi ke Perusahaan</label>
                        <textarea class="form-control" id="suggestions_for_company" name="suggestions_for_company" rows="3"
                            placeholder="Tuliskan saran untuk pengelolaan magang.">{{ old('suggestions_for_company') }}</textarea>
                    </div>
            ...
            <div class="logs-card">
                <div class="logs-title">Riwayat Laporan Akhir</div>
                @if ($reports->isEmpty())
                            ...
                            <tbody>
                                @foreach ($reports as $report)
                                    <tr>
                                        ...
                                        <td>
                                            <a target="_blank" rel="noopener" href="{{ route('final-report.view', $report->id) }}">
                                                Lihat
                                            </a>
                                            |
                                            <a href="{{ route('final-report.download', $report->id) }}">
                                                {{ $report->file_name }}
                                            </a>
                                        </td>
                                        <td>
                                            <div><strong>Dipelajari:</strong> {{ \Illuminate\Support\Str::limit($report->what_learned ?: '-', 80) }}</div>
                                            <div><strong>Kendala:</strong> {{ \Illuminate\Support\Str::limit($report->challenges ?: '-', 80) }}</div>
                                        </td>
                    ...
                    <div class="d-md-none">
                        @foreach ($reports as $report)
                            <div class="border rounded p-2 mb-2 bg-white">
                                ...
                                <div class="small mt-1"><strong>Dipelajari:</strong> {{ \Illuminate\Support\Str::limit($report->what_learned ?: '-', 80) }}</div>
                                <div class="small mt-1"><strong>Kendala:</strong> {{ \Illuminate\Support\Str::limit($report->challenges ?: '-', 80) }}</div>
                                <div class="small mt-1">
                                    <a target="_blank" rel="noopener" href="{{ route('final-report.view', $report->id) }}">Lihat</a>
                                    |
                                    <a href="{{ route('final-report.download', $report->id) }}">{{ $report->file_name }}</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
